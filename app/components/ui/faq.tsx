...
const FAQ: React.FC = () => {
	...
	const faqData = [
		...
		{
			question:
				'How is Resume Tailor different from other resume tailoring tools?',
			answer:
		...
		},
		{
			...
			answer:
				"Horizon's Resume Tailor allows you to tailor 2 resumes with AI support. For unlimited resume tailoring and additional features, you can upgrade to Resume Tailor Pro for $4.99/month.",
		},
		{
	...
	useEffect(() => {
		const newMaxHeights = refs.map(ref =>
			ref.current ? `${ref.current.scrollHeight}px` : '0px',
		)
