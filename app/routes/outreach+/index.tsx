...
type ActionError = {
  ...
  errors?: Partial<Record<"jobTitle" | "jobDescription" | "resume" | "imports", string>>;
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  if (!userId) return json({ userId: null, subscription: null, freeRemaining: null });
...
function getText(fd: FormData, key: string, { trim = true, max = 100_000 } = {}) {
  const v = fd.get(key);
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  const username = typeof usernameRaw === "string" ? usernameRaw.trim() || undefined : undefined;
    ...
    return json<ActionError>(
      {
        ok: false,
        errors,
        fields: { jobTitle, jobDescription, resume, recruiterName, name, username }
      },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    if (!subscription) {
      return json<ActionError>(
        {
          ok: false,
          requireSubscribe: true, // ✅ fixed typo (was reqireSubscribe)
    ...
    const progress = await prisma.gettingStartedProgress.upsert({
      where: { ownerId: userId },
      update: {},
      create: { ownerId: userId, outreachCount: 0 },
      select: { outreachCount: true }
    });
    ...
    try {
      ...
      outreach = (await writeRecruiterMessage({
        jobTitle,
        jobDescription,
        resume,
        recruiterName,
...
export default function RecruiterOutreachPage() {
  ...
  useEffect(() => {
    if (data && "fields" in data && data.fields) {
      ...
      setResumeText(localStorage.getItem("recruiter-resume") || localStorage.getItem("resume-draft-outreach") || "");
      ...
      <section className="mx-auto max-w-5xl px-6 pt-12">
        <div className="rounded-3xl border bg-white/70 p-8 shadow-sm backdrop-blur">
          ...
          <p className="mt-3 max-w-3xl text-base text-gray-600">
            Paste your <span className="font-medium text-gray-900">resume</span> and the
            <span className="font-medium text-gray-900"> job description</span>, add the
            recruiter’s name, and we’ll craft polished <span className="font-medium text-gray-900">email & LinkedIn messages</span>
            you can send today to land more interviews.
          </p>
          <ul className="mt-4 grid gap-2 text-sm text-gray-600 sm:grid-cols-3">
            <li className="flex items-center gap-2"><span className="inline-block h-1.5 w-1.5 rounded-full bg-green-500" /> Tailored to the role</li>
            <li className="flex items-center gap-2"><span className="inline-block h-1.5 w-1.5 rounded-full bg-blue-500" /> Multiple message options</li>
            <li className="flex items-center gap-2"><span className="inline-block h-1.5 w-1.5 rounded-full bg-purple-500" /> Copy & send instantly</li>
          </ul>
        </div>
      </section>
      ...
      <main className="mx-auto mt-8 grid max-w-5xl gap-6 px-6 md:grid-cols-5">
        ...
        <div className="md:col-span-3">
          <div className="rounded-2xl border bg-white p-6 shadow-sm">
            ...
            <p className="mt-1 text-sm text-gray-600">We don’t store your content. Use .txt/.md uploads or paste the text.</p>

            <Form method="post" className="mt-6 space-y-5" replace onSubmit={handleAuthSubmit}>
              <div>
                ...
                <input
                  ...
                  className="mt-1 w-full rounded-md border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-black/10"
                  aria-invalid={!!err?.jobTitle}
                  value={jobTitle}
                  onChange={(e) => setJobTitle(e.target.value)}
                />
                {err?.jobTitle && <p className="mt-1 text-sm text-red-600">{err.jobTitle}</p>}
              </div>

              <div>
                <div className="flex items-center justify-between">
                  <label htmlFor="recruiterName" className="block text-sm font-medium">Recruiter or hiring manager</label>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="mt-1 w-full rounded-md border px-3 py-2 font-mono text-sm focus:outline-none focus:ring-2 focus:ring-black/10"
                  aria-invalid={!!err?.jobDescription}
                  value={jdText}
                  onChange={(e) => setJdText(e.target.value)}
                />
                {err?.jobDescription && <p className="mt-1 text-sm text-red-600">{err.jobDescription}</p>}
              </div>
              ...
              <details className="rounded-md border p-3">
                <summary className="cursor-pointer text-sm font-medium">Optional: personalize messages with your name</summary>
                <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div>
                    <label htmlFor="name" className="block text-sm font-medium">Your name</label>
                    <input id="name" name="name" type="text" className="mt-1 w-full rounded-md border px-3 py-2 text-sm" placeholder="Alex Rivera" defaultValue={(data as any)?.fields?.name || ""} />
                  </div>
                  <div>
                    <label htmlFor="username" className="block text-sm font-medium">Username (optional)</label>
                    <input id="username" name="username" type="text" className="mt-1 w-full rounded-md border px-3 py-2 text-sm" placeholder="arivera" defaultValue={(data as any)?.fields?.username || ""} />
                  </div>
                </div>
              </details>
              ...
              <button
                type="submit"
                className="inline-flex w-full items-center justify-center rounded-md bg-black px-4 py-2.5 text-white shadow-sm transition hover:opacity-90 disabled:opacity-50"
        ...
        <div className="md:col-span-2">
          <div className="sticky top-6">
            <div className="rounded-2xl border bg-white p-6 shadow-sm">
              ...
              <p className="mt-1 text-sm text-gray-600">Email subject lines, email body, LinkedIn DM, connection note, and a follow-up you can send.</p>
                ...
                <div className="mt-6 space-y-4 text-sm text-gray-600">
                  <div className="rounded-md border bg-gray-50 p-4">
                    ...
                    <ul className="mt-2 list-disc pl-5">
                      ...
                      <li>Click <span className="font-medium">Generate outreach</span> to get messages tailored to the role.</li>
                    </ul>
                  </div>
                  <div className="rounded-md border bg-gray-50 p-4">
                    <div className="font-medium text-gray-900">Why it helps</div>
                    <ul className="mt-2 list-disc pl-5">
              ...
              {success && (
                <div className="mt-6 space-y-6 text-sm">
                    ...
                    {success.outreach?.email?.subjects?.length ? (
                      <ul className="list-disc pl-5">
                        {success.outreach.email.subjects.map((s, i) => (
                          <li key={i} className="mb-1 flex items-start justify-between gap-3">
                            <span>{s}</span>
                            <CopyButton text={s} />
                          </li>
                        ))}
                      ...
                      <p className="text-gray-600">No subject suggestions returned.</p>
                    )}
                  </section>
                  ...
                  <section>
                    ...
                    <p className="whitespace-pre-wrap rounded-md border bg-gray-50 p-3">{success.outreach?.email?.body || "—"}</p>
                  </section>
                  ...
                  <section>
                    ...
                    <p className="whitespace-pre-wrap rounded-md border bg-gray-50 p-3">{success.outreach?.linkedinDM?.body || "—"}</p>
                  </section>
                  ...
                  <section>
                    <div className="mb-2 flex items-center justify-between">
                      ...
                      <CopyButton text={success.outreach?.connectionNote?.body} />
                    </div>
                    <p className="whitespace-pre-wrap rounded-md border bg-gray-50 p-3">{success.outreach?.connectionNote?.body || "—"}</p>
                  </section>
                  ...
                  <section>
                    ...
                    <p className="whitespace-pre-wrap rounded-md border bg-gray-50 p-3">{success.outreach?.followUp?.body || "—"}</p>
                  </section>
                </div>
              )}
            </div>
