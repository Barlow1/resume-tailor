...
function sectionHeader(title: string, accentColor: string, fontFamily: string, ts: (n: number) => number, options: TemplateOptions, headerField: string, placeholder: string): string {
	// Outdented: margin-left pulls the label into the gutter
	return `<div style="font-size: ${ts(11)}px; font-weight: 700; color: ${accentColor}; text-transform: uppercase; letter-spacing: 0.14em; margin-bottom: 10px; margin-left: -22px; font-family: ${fontFamily};" ${editableAttrs(options, headerField, placeholder)}>${escapeHtml(title)}</div>`
}
...
export function generate(
	...
	const font = theme.bodyFont // Space Grotesk for everything
	...
	const contactHtml = visibleContacts.length > 0 || options.editable
		? `<div style="font-size: ${ts(13)}px; color: #555; margin-top: 6px; font-family: ${font};">${visibleContacts.map((c, idx) => {
	...
	// Stark header with bold rule
	const headerHtml = `
		<div style="margin-bottom: 28px; padding-bottom: 16px; border-bottom: 3px solid ${accent};">
			<div style="font-size: ${ts(28)}px; font-weight: 700; color: #000; font-family: ${font}; line-height: 1.1;" ${editableAttrs(options, 'name', 'Your Name')}>${escapeHtml(formData.name || '')}</div>
			${(formData.role || options.editable) ? `<div style="font-size: ${ts(16)}px; color: #444; margin-top: 4px; font-family: ${font};" ${editableAttrs(options, 'role', 'Job Title')}>${escapeHtml(formData.role || '')}</div>` : ''}
			${contactHtml}
		</div>`
	...
	function summarySection(): string {
		...
		<div style="margin-bottom: 22px;" data-section-id="about">
			${sectionHeader(h, accent, font, ts, options, 'headers.aboutHeader', 'Summary')}
			<div style="font-size: ${ts(16)}px; line-height: 1.6; color: #333; font-family: ${font};" ${editableAttrs(options, 'about', 'Write a professional summary...', { multiline: true })}>${escapeHtml(formData.about || '')}</div>
		</div>`
	}
	...
	function experienceSection(): string {
		...
		<div style="margin-bottom: 22px;" data-section-id="experience">
			${sectionHeader(h, accent, font, ts, options, 'headers.experienceHeader', 'Experience')}
			${exps.map((exp, i) => {
			...
			<div style="margin-bottom: 16px; break-inside: avoid;" ${options.editable ? `data-experience-id="${exp.id}"` : ''}>
				<div style="display: flex; justify-content: space-between; align-items: baseline;">
					<div>
						<span style="font-size: ${ts(16)}px; font-weight: 700; color: #111; font-family: ${font};" ${editableAttrs(options, `experiences.${i}.role`, 'Job Title')}>${escapeHtml(exp.role || '')}</span>
						${(exp.company || options.editable) ? `<span style="font-size: ${ts(15)}px; color: #555; font-style: italic; font-family: ${font}; margin-left: 6px;" ${editableAttrs(options, `experiences.${i}.company`, 'Company Name')}>${escapeHtml(exp.company || '')}</span>` : ''}
					</div>
					${(exp.startDate || exp.endDate || options.editable) ? `<span style="font-size: ${ts(12.5)}px; color: #888; font-style: italic; font-family: ${font}; flex-shrink: 0; white-space: nowrap;"><span ${editableAttrs(options, `experiences.${i}.startDate`, 'Start')}>${escapeHtml(exp.startDate || '')}</span>${(exp.startDate || exp.endDate || options.editable) ? ' \u2013 ' : ''}<span ${editableAttrs(options, `experiences.${i}.endDate`, 'End')}>${escapeHtml(exp.endDate || '')}</span></span>` : ''}
				</div>
				${bullets.length > 0 ? `
				<ul style="margin: 6px 0 0; padding-left: 18px; list-style-type: square;">
					${bullets.map((b, j) => `<li style="font-size: ${ts(16)}px; line-height: 1.55; color: #333; margin-bottom: 3px; font-family: ${font};" ${options.editable ? `data-bullet-index="${j}"` : ''} ${b.id ? `data-description-id="${b.id}"` : ''}><span ${editableAttrs(options, `experiences.${i}.descriptions.${j}.content`, 'Describe what you did...', { multiline: true })}>${escapeHtml(b.content || '')}</span></li>`).join('\n\t\t\t\t\t')}
				</ul>` : ''}
			</div>`
			}).join('\n')}
		</div>`
	...
	function educationSection(): string {
		...
		<div style="margin-bottom: 22px;" data-section-id="education">
			${sectionHeader(h, accent, font, ts, options, 'headers.educationHeader', 'Education')}
			${edu.map((e, i) => `
			<div style="display: flex; justify-content: space-between; margin-bottom: 10px; break-inside: avoid;" ${options.editable ? `data-education-id="${e.id}"` : ''}>
				<div>
					<div style="font-size: ${ts(16)}px; font-weight: 700; color: #111; font-family: ${font};" ${editableAttrs(options, `education.${i}.school`, 'School Name')}>${escapeHtml(e.school || '')}</div>
					<div style="font-size: ${ts(15)}px; color: #555; font-style: italic; font-family: ${font};" ${editableAttrs(options, `education.${i}.degree`, 'Degree')}>${escapeHtml(e.degree || '')}</div>
				</div>
				${(e.startDate || e.endDate || options.editable) ? `<span style="font-size: ${ts(12.5)}px; color: #888; font-style: italic; font-family: ${font}; white-space: nowrap;"><span ${editableAttrs(options, `education.${i}.startDate`, 'Start')}>${escapeHtml(e.startDate || '')}</span>${(e.startDate || e.endDate || options.editable) ? ' \u2013 ' : ''}<span ${editableAttrs(options, `education.${i}.endDate`, 'End')}>${escapeHtml(e.endDate || '')}</span></span>` : ''}
			</div>`).join('\n')}
		</div>`
	}
	...
	function skillsSection(): string {
		...
		<div style="margin-bottom: 22px;" data-section-id="skills">
			${sectionHeader(h, accent, font, ts, options, 'headers.skillsHeader', 'Skills')}
			${skills.map((s, i) => `<div style="font-size: ${ts(16)}px; color: #333; line-height: 1.6; font-family: ${font};" ${options.editable ? `data-skill-id="${s.id}"` : ''} ${editableAttrs(options, `skills.${i}.name`, 'Skill')}>${escapeHtml(s.name || '')}</div>`).join('\n\t\t\t')}
		</div>`
	}
	...
	function hobbiesSection(): string {
		...
		<div style="margin-bottom: 22px;" data-section-id="hobbies">
			${sectionHeader(h, accent, font, ts, options, 'headers.hobbiesHeader', 'Interests')}
			${hobbies.map((hob, i) => `<div style="font-size: ${ts(16)}px; color: #333; line-height: 1.6; font-family: ${font};" ${options.editable ? `data-hobby-id="${hob.id}"` : ''} ${editableAttrs(options, `hobbies.${i}.name`, 'Interest')}>${escapeHtml(hob.name || '')}</div>`).join('\n\t\t\t')}
		</div>`
	}
